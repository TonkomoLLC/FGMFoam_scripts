...
AUX_SOURCE_FIELDS = ["YWI", "YuWI", "YbWI"]
AUX_BOUND_FIELDS = ["Yu2I", "YuYbI", "Yb2I"]
...
def main() -> None:
    ...
    # Auxiliary tables required by FGMModel when useProgressVariableVariance=true.
    # This is the zero-variance (delta-PDF) identity, consistent with the
    # corresponding zero-variance slices in the supplied developer table set.
    pv_raw = pvmin[:, None] + (pvmax - pvmin)[:, None] * cgrid[None, :]
    aux_source = {
        "YWI": pv_raw * fields["SourcePV"],
        "YuWI": pvmin[:, None] * fields["SourcePV"],
        "YbWI": pvmax[:, None] * fields["SourcePV"],
    }
    aux_bounds = {
        "Yu2I": pvmin * pvmin,
        "YuYbI": pvmin * pvmax,
    ...
    # Verify the algebra used in FGMModel.C: for varPV=0, these auxiliary
    # moments must yield scaledVarPV=0 to floating-point tolerance.
    gc = aux_bounds["YuYbI"] - aux_bounds["Yu2I"]
    hc = aux_bounds["Yb2I"] - 2.0 * aux_bounds["YuYbI"] + aux_bounds["Yu2I"]
    good = hc > 1.0e-14
    zero_variance_residual = np.zeros_like(pv_raw)
    if np.any(good):
        zero_variance_residual[good, :] = (
            (
                pv_raw[good, :]**2
                - aux_bounds["Yu2I"][good, None]
                - 2.0*gc[good, None]*cgrid[None, :]
            ) / hc[good, None]
            - cgrid[None, :]**2
        )
    max_zero_variance_residual = float(np.max(np.abs(zero_variance_residual[good, :]))) if np.any(good) else 0.0
    if max_zero_variance_residual > 1.0e-10:
        raise SystemExit(f"Zero-variance auxiliary identity failed: max residual={max_zero_variance_residual:.6e}")
    ...
    meta = {
        ...
        "variance": "Auxiliary tables support useProgressVariableVariance true only in the zero-variance limit. Variance slices remain replicated, not PDF-integrated.",
        "auxiliary_definitions": {"YWI": "PV*SourcePV", "YuWI": "PVmin*SourcePV", "YbWI": "PVmax*SourcePV", "Yu2I": "PVmin^2", "YuYbI": "PVmin*PVmax", "Yb2I": "PVmax^2"},
        "zero_variance_identity_max_residual": max_zero_variance_residual,
    ...
    with tarfile.open(output, "w:xz") as tf:
        ...
        for name, arr in aux_source.items():
            add_csv(tf, f"02_tables/extras/{name}.csv", arr)
        for name, arr in aux_bounds.items():
            add_csv(tf, f"02_tables/extras/{name}.csv", arr.reshape(1, -1))
    ...
    print(f"[OK] Full zero-variance auxiliary table interface written; scaledVarPV identity residual={max_zero_variance_residual:.6e}")
