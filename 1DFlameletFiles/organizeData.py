...
def main():
    ...
    for fn in files:
        ...
        # --- chemistry source term for PV ---
        # Cantera net production rates: kmol/m^3/s for each species
        SourcePV = np.empty(n, dtype=float)

        for i in range(n):
            gas.TPY = T[i], P, Y[i, :]
            wdot = gas.net_production_rates  # kmol/m^3/s, length nspecies
        
            # PV definition uses n_k = Y_k / MW_k (kmol/kg)
            # so PVdot_chem = sum(coeff_k * wdot_k / rho) (kmol/kg/s)
        ...
        # final guard (optional, redundant if you do per-value guard above)
        SourcePV = np.where(np.isfinite(SourcePV), SourcePV, 0.0)
            
        # --- write output ---
        header = ['z','u','V','T','rho','Z','PV','SourcePV','chiZ','S_n'] \
                 + [f'Y.{s}' for s in gas.species_names]
        out = np.column_stack([z, u, V, T, rho, Z, PV, SourcePV, chiZ, S_n, Y])
