...
python3 "$SCRIPTS_DIR/validate_of7_premixed_tables.py" --case "$CASE_DIR" \
    --require-species "${REQUIRED_SPECIES[@]}" --check-z "$PILOT_Z" "$Z_MAX" --pilot-z "$PILOT_Z" --require-zero-input-variances
...
settings = {'useProgressVariableVariance': 'true', 'useMixtureFractionVariance': 'false'}
for key, value in settings.items():
    pat = rf'(?m)^(\s*)(?!//)({key}\s+)(true|yes|on|false|no|off)(\s*;)'
    s, n = re.subn(pat, rf'\g<1>\g<2>{value}\g<4>', s)
    if n == 0:
        print(f'[WARN] Active {key} not found in {p}; add {key} {value}; in FGMModelCoeffs')
p.write_text(s)
print(f'[OK] Set useProgressVariableVariance true; retained useMixtureFractionVariance false where present in {p}')
PY
else
...
echo "[DONE] useProgressVariableVariance true enabled for the validated zero-variance auxiliary-table path."
