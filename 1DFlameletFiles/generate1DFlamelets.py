#!/usr/bin/env python3
# generate1DFlamelets.py — Cantera 3.1 or 3.2 (script v3.0)
...
def seed_hotspot(f, T_peak=1700.0, center=0.55, halfwidth=0.12):
    ...
    # Cantera 3.1+ preferred API: set the profile on the flame Domain1D
    set_profile_T(f, xi, prof)

import warnings

def set_profile_T(f, xi, prof):
    # Cantera 3.2+: Domain1D has set_profile (preferred; no warning)
    if hasattr(f, "flame") and hasattr(f.flame, "set_profile"):
        f.flame.set_profile("T", xi, prof)
        return

    # Cantera 3.1 (and older): only Sim1D.set_profile exists -> will warn unless suppressed
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
            category=DeprecationWarning,
            message=r"Sim1D::setProfile:.*",
        )
